...
type Tab = "overview" | "media" | "use-cases" | "architecture" | "downloads" | "faqs" | "seo";
...
export function FeatureCmsClient({ feature }: Props) {
  const router = useRouter();
  const productId = feature.product.id;
  ...
  // Media state
  const [mediaItems, setMediaItems] = useState<MediaItem[]>(feature.mediaItems);
  ...
  // Use Cases state
  const [useCases, setUseCases] = useState<UseCase[]>(feature.useCases);
  ...
  // Architecture state
  const [archSections, setArchSections] = useState<ArchSection[]>(feature.architectureSections);
  ...
  // Downloads state
  const [downloads, setDownloads] = useState<Download[]>(feature.downloads);
  ...
  // FAQs state
  const [faqs, setFaqs] = useState<Faq[]>(feature.faqs);
  ...
  async function saveOverview() {
    if (!featureOverview.name.trim()) return;
    setOverviewSaving(true);
    setOverviewMsg(null);
    try {
      const res = await fetch(`/api/cms/products/${productId}/features/${featureId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: featureOverview.name,
          overview: featureOverview.overview,
          details: featureOverview.details,
          coverImage: featureOverview.coverImage,
          status: featureOverview.status,
          order: Number(featureOverview.order),
        }),
      });
      if (!res.ok) throw new Error("Failed to save");
      setOverviewMsg({ type: "success", text: "Feature overview saved successfully." });
    } catch {
      setOverviewMsg({ type: "error", text: "Failed to save feature overview." });
    } finally {
  ...
  async function addMedia() {
    ...
    setMediaMsg(null);
    try {
      const res = await fetch(`/api/cms/features/${featureId}/media`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function addUseCase() {
    ...
    setUseCaseMsg(null);
    try {
      const res = await fetch(`/api/cms/features/${featureId}/use-cases`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function addArch() {
    ...
    setArchMsg(null);
    try {
      const res = await fetch(`/api/cms/features/${featureId}/architecture`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setArchMsg({ type: "success", text: "Architecture section added." });
    } catch {
  ...
  async function addDownload() {
    ...
    setDownloadMsg(null);
    try {
      const res = await fetch(`/api/cms/features/${featureId}/downloads`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function addFaq() {
    ...
    setFaqMsg(null);
    try {
      const res = await fetch(`/api/cms/features/${featureId}/faqs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteFeature() {
    if (!confirm(`Are you sure you want to permanently delete "${feature.name}"? This action cannot be undone.`)) return;
    setDeleting(true);
    setDeleteMsg(null);
    try {
      const res = await fetch(`/api/cms/products/${productId}/features/${featureId}`, {
      ...
      router.push(`/admin/cms/products/${productId}`);
    } catch {
      setDeleteMsg({ type: "error", text: "Failed to delete feature. Please try again." });
      setDeleting(false);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{feature.name}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{feature.product.name}</p>
        </div>
        <Badge
          variant={
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            <h2 className="font-semibold text-slate-900 dark:text-white mb-4">Feature Overview</h2>
            <div>
              <label className={labelCls}>Name *</label>
              <input
                type="text"
                className={inputCls}
                value={featureOverview.name}
                onChange={(e) => setFeatureOverview((prev) => ({ ...prev, name: e.target.value }))}
            ...
            <div>
              ...
              <textarea
                rows={3}
                className={inputCls}
                value={featureOverview.overview}
                onChange={(e) => setFeatureOverview((prev) => ({ ...prev, overview: e.target.value }))}
            ...
            <div>
              ...
              <textarea
                rows={6}
                className={inputCls}
                value={featureOverview.details}
                onChange={(e) => setFeatureOverview((prev) => ({ ...prev, details: e.target.value }))}
                placeholder="Detailed description, technical specs, etc..."
              />
            </div>
            <div>
              ...
              <input
                type="url"
                className={inputCls}
                value={featureOverview.coverImage}
                onChange={(e) => setFeatureOverview((prev) => ({ ...prev, coverImage: e.target.value }))}
                placeholder="https://..."
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  className={inputCls}
                  value={featureOverview.status}
                  onChange={(e) => setFeatureOverview((prev) => ({ ...prev, status: e.target.value }))}
                >
              ...
              <div>
                ...
                <input
                  type="number"
                  className={inputCls}
                  value={featureOverview.order}
                  onChange={(e) => setFeatureOverview((prev) => ({ ...prev, order: Number(e.target.value) }))}
                  min={0}
                />
              </div>
            </div>
            <div className="pt-2">
              <button onClick={saveOverview} disabled={overviewSaving || !featureOverview.name.trim()} className={btnPrimary}>
        ...
        {activeTab === "media" && (
          <div>
            ...
            <div className="border border-slate-200 dark:border-slate-600 rounded-lg p-4 mb-6 space-y-3 bg-slate-50 dark:bg-slate-900/50">
              ...
              <button onClick={addMedia} disabled={mediaSubmitting || !newMedia.url.trim()} className={btnPrimary}>
        ...
        {activeTab === "architecture" && (
          <div>
            ...
            <div className="border border-slate-200 dark:border-slate-600 rounded-lg p-4 mb-6 space-y-3 bg-slate-50 dark:bg-slate-900/50">
              ...
              <button onClick={addArch} disabled={archSubmitting || !newArch.title.trim()} className={btnPrimary}>
                ...
                {archSections.map((a) => (
                  <li key={a.id} className="py-3 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      {a.imageUrl && <p className="text-xs text-slate-400 mt-0.5 truncate">{a.imageUrl}</p>}
                    </div>
                ...
                {downloads.map((d) => (
                  <li key={d.id} className="py-3 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      {d.fileSize && <p className="text-xs text-slate-400 mt-0.5">{d.fileSize}</p>}
                    </div>
      ...
      {/* Danger Zone */}
      <div className="mt-8 bg-white dark:bg-slate-800 rounded-xl border border-red-200 dark:border-red-900/50 p-6">
        <h3 className="font-semibold text-red-700 dark:text-red-400 mb-2">Danger Zone</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          Permanently delete this feature and all of its associated content. This action cannot be undone.
        </p>
        <button
